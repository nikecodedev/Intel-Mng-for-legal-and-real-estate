  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? <p className="text-sm text-gray-500">Carregando...</p> : !data || data.length === 0 ? (
        <p className="text-sm text-gray-500">Nenhum ativo disponível.</p>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data.map((a: any) => (
            <Link key={a.id} href={`/investor/assets/${a.id}`} className="rounded-lg border border-gray-200 bg-white p-5 hover:border-blue-300 hover:shadow-sm transition-all">
              <h3 className="font-medium text-gray-900">{a.title || a.asset_reference || `Ativo ${a.id.slice(0, 8)}`}</h3>
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                {a.risk_score != null && <span className="text-xs text-gray-500">Risco: {a.risk_score}%</span>}
              </div>
            </Link>
          ))}
        </div>
